...
本分支重点解决旧版长期未更新后出现的兼容性、文件安全和交互问题。当前版本为 **1.0.2**，Univer 已由 **0.2.14** 升级至 **0.25.1**。

> [!IMPORTANT]
> `.xlsx` 的目标是安全处理常用工作簿，而不是宣称完整复刻 Microsoft Excel。插件会在写回可能造成数据丢失时自动进入保护视图。
...
| 版本线 | 原项目版本 | 从 `1.0.0` 重新开始，当前 `1.0.2` |
...
- **保护视图**：写回前检查不受支持的 Excel 部件，避免静默删除图表、宏等内容。
- **首次保存备份**：默认在每个编辑会话第一次保存前备份原文件，位置为 `<vault>/.obsidian/plugins/univer-plus/backups`。
- **外部修改检测**：文件在编辑期间被其他程序改动时，阻止无提示覆盖。
- **保存协调**：自动保存与手动保存串行执行，降低竞态覆盖风险。
- **空工作簿保护**：原本有内容的工作簿意外变为空时拒绝自动保存；确需清空时，连续执行两次 `Ctrl/Cmd+S` 确认。
...
2. 选择 Sheet、Doc 或 Excel 创建对应文件。
3. 也可以直接在文件列表中打开已有 `.usheet`、`.udoc` 或 `.xlsx` 文件。
4. `.xlsx` 文件包含不受支持的内容时，顶部状态会显示保护视图；点击状态可查看原因。
...
- **Create backup before first save**：控制每个编辑会话第一次保存前是否创建备份，默认开启。
