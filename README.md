...
本分支重点解决旧版长期未更新后出现的兼容性、文件安全和交互问题。当前版本为 **1.0.3**，Univer 已由 **0.2.14** 升级至 **0.25.1**。
...
| 版本线 | 原项目版本 | 从 `1.0.0` 重新开始，当前 `1.0.3` |
...
含复杂 Word 资源的文档会以保护视图打开：可以浏览、选择和复制文本，但不会覆盖原文件。这样不会因为当前转换器无法表达图片、表格或修订内容而破坏源文档。
...
- `.docx` 目前侧重基础文字文档；复杂 Word 资源只能在保护视图中预览文本。
